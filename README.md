<p align="center">
  ...
  <h3 align="center">ata</h3>
  <p align="center">A go binaries for parallel task.</p>
</p>
...
### -t
如果要对整个ata程序所在进程的资源做限制,可设置`-t`参数,指定最多同时并行多少个子进程
...
# input.sh.db数据库
ata会针对每一个输入脚本，在脚本所在目录生成`脚本名称`+`.db`的sqlite3数据库，用于记录各`子脚本`的运行状态，例如`input.sh`对应的数据库名称为`input.sh.db`
...
*  **exitCode**为对应子脚本的退出码
*  **retry**为如果子脚本出错的情况下ata程序自动重新尝试运行该出错子进程的次数（目前还未启用此功能）
...
`work.sh`时，ata会自动跳过已经成功完成的子脚本，只运行出错的子脚本。

### 要在alpine docker中运行怎么办
> alpine 镜像默认不带sqlite3，ata依赖于sqlite3，alpine需要更新，Dockerfile可以参考下面的
...
这样就能直接在docker容器内的命令行使用ata，而不必写绝对路径了
